...
function barchart() {
  ...
  d3.csv(
    "json/publictransport" + years + ".csv",
    parser,
    function (error, data) {
      state_accept(data);
    }
...
function colorTran(name) {
  ...
  d3.selectAll("path")
    ...
    .on("mouseover", function (d) {
      tooltip.transition().duration(150).style("opacity", 0.7);
      tooltip
        .html("" + d.Region + "<br>" + name + ": " + d[name] + " thousands")
...
var color = d3.scaleLinear().range([schLow, schMed, schHigh]);

var colorValueAcc = d3.scaleLinear().range([envLow, envMed, envHigh]);

function colorAccidents() {
  function parser(d) {
    d.totalacc = +d.totalacc;
    //console.log(d.State_name)
    if (d.years == years) return d;
  }
...
function plotTransportations() {
  console.log("here");
  d3.json("json/uk2.json", function (topology) {
    smap
      .attr("id", "accidents")
      .selectAll(".state")
...
function barchartDummy(yea, reg) {
  if (!visible) {
    function parser(d) {
      d.tot = +d.tot;
      //console.log(d.State_name)
      if (d.label == reg && d.years == yea) return d;
    }
    d3.csv("json/ur.csv", parser, function (error, data) {
      ...
      var y = d3
        .scaleOrdinal()
        .range([height, 0]) // Use range instead of rangeRound
        .domain(
          data.map(function (d) {
            return d.urbanrural;
          })
      ...
      var gy = smap2.append("g").attr("class", "y axis").call(yAxis);

      var bars = smap2.selectAll(".bar").data(data).enter().append("g");
      ...
      //   })
      //   .attr("height", y.bandwidth()) // Use y.bandwidth() for the height
      //   .attr("x", function (d) {
      //     return y(d.tot);
      //   })
      ...
      bars
        ...
        .attr("class", "label")
        .attr("y", function (d) {
          return y(d.urbanrural) + y.bandwidth() / 2 + 4;
        })
        .attr("x", "200")
        .text(function (d) {
          return "Total: " + d.tot;
  ...
  if (!fvisible) {
    ...
    d3.csv("json/sev.csv", parser, function (error, data) {
      ...
      bars
        ...
        .attr("class", "label")
        .attr("y", function (d) {
          return y(d.sev) + y.bandwidth() / 2 + 4;
        })
        .attr("x", "200")
        .text(function (d) {
          return "Total: " + d.tot;
